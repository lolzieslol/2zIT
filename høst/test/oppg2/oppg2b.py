...
# Skriv ut alle data på ein organisert måte, eksempelvis slik som i vedlegg (der du hentar data om produkta frå).
def printAllDictInfo(dictlist):
    for i in range(len(dictlist)):
        for value in dictlist.values():
            for verdi in value.values():
                if type(verdi) != list:
                    print(verdi)
                else:
...
# printAllDictInfo(Lager)


# -	Skriv ein funksjon som tekk inn vare-ID og deretter skriv ut all informasjon om produktet. Feilmelding om den gitte vara ikkje finnes.
def UtskriftVareFraLager(varetittel, dictlist):
    # print("utskrift start")
    # if dictlist[varetittel]["varetittel"]:
    #         a = 1
    #         for key in dictlist.keys().keys():
    #             print(key)
    # else:
    #     print(f"varen, {varetittel} var ikke i listen")
    if varetittel in dictlist.keys():
            for verdi in dictlist[varetittel].values():
                if type(verdi) != list:
                    print(verdi)
                else:
                    for i in range(len(verdi)):
...
UtskriftVareFraLager(vareinp, Lager)
